...
import { Crown, User, Award, Star, Stethoscope } from "lucide-react";
      ...
      <div className="relative z-10 p-6 md:p-10">
        ...
        <div className="flex flex-col sm:flex-row gap-5 md:gap-8 items-start sm:items-center">
          ...
          <div className="relative flex-shrink-0 self-center sm:self-auto">
            <div className="w-20 h-20 sm:w-28 md:w-36 sm:h-28 md:h-36 rounded-2xl md:rounded-3xl bg-white/20 backdrop-blur-sm border-2 border-white/30 flex items-center justify-center text-white font-serif font-bold text-2xl sm:text-4xl md:text-5xl shadow-xl group-hover:scale-105 transition-transform duration-500">
              {doctor.initials}
            </div>
            {/* Crown badge */}
            <div className="absolute -top-2 -right-2 md:-top-3 md:-right-3 bg-[hsl(38,95%,52%)] text-white rounded-full w-8 h-8 md:w-10 md:h-10 flex items-center justify-center shadow-lg animate-glow-accent">
              <Crown className="w-4 h-4 md:w-5 md:h-5" />
            </div>
            {/* Orbiting dot — desktop only */}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-1.5 md:gap-2">
              {doctor.specializations.map((spec) => (
                <span
                  key={spec}
                  className="px-2 md:px-3 py-0.5 md:py-1 rounded-full bg-white/15 text-white text-[10px] md:text-xs font-medium border border-white/15 backdrop-blur-sm"
                >
                  {spec}
                </span>
              ))}
        ...
        {/* Mobile star rating (compact inline) */}
    ...
    <Reveal direction={dir} delay={index * 130} duration={850} distance={45}>
      <div className="group glass-card-strong rounded-2xl overflow-hidden hover:-translate-y-1 md:hover:-translate-y-2 hover:shadow-[var(--shadow-card-hover)] transition-all duration-500 h-full cursor-default relative">
        ...
        <div
          ...
          style={{ background: `linear-gradient(90deg, ${doctor.gradientFrom}, ${doctor.gradientTo})` }}
        />
        ...
        <div className="p-5 md:p-7">
          ...
          <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-5">
            <div
              className="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl flex-shrink-0 flex items-center justify-center text-white font-serif font-bold text-base md:text-xl shadow-md group-hover:scale-105 transition-transform duration-300"
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
                ...
                <span
                  ...
                  title={doctor.gender === "female" ? "Female Physiotherapist" : "Male Physiotherapist"}
                >
          ...
          {/* Experience badge */}
          <div
            className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] md:text-xs font-semibold mb-3 md:mb-4"
          ...
          {/* Bio — hide on very small screens for brevity */}
          <p className="hidden sm:block text-muted-foreground text-xs md:text-sm leading-relaxed mb-4 md:mb-5">
            {doctor.bio}
          </p>
          ...
          <div className="flex flex-wrap gap-1">
            {doctor.specializations.map((spec) => (
              <span
                ...
                style={{
                  background: `hsla(${doctor.color}, 0.07)`,
                  color: `hsl(${doctor.color})`,
                  borderColor: `hsla(${doctor.color}, 0.2)`,
                }}
              >
                {spec}
              </span>
  ...
  return (
    ...
    >
      ...
      <div className="container relative">
        ...
        <Reveal direction="up" duration={900} distance={45}>
          <div className="text-center max-w-2xl mx-auto mb-8 md:mb-12 px-2">
            ...
            <p className="text-muted-foreground mt-3 md:mt-4 text-sm md:text-base leading-relaxed">
              A dedicated team of certified physiotherapists committed to your
              recovery, comfort, and long-term wellbeing.
            </p>
          </div>
        </Reveal>
        ...
        {/* Rest — 1-col mobile, 2-col tablet, 3-col desktop */}
